...
trawllight::tlu_get_casts(
  directory_structure = data.frame(path = read.csv(file = here::here("imports", "directories.csv"))[9:10,]),
  survey = survey,
  cast.dir = "downcast",
  time.buffer = 20,
  silent = FALSE,
...
trawllight::tlu_get_surface(
  data.frame(path = read.csv(file = here::here("imports", "directories.csv"))[9:10,]),
  survey = survey, 
  time.buffer = 30, 
  agg.fun = trawllight::geometric.mean)
```
