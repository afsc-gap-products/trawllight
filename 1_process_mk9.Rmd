...
channel <- mk9process:::get_connected(schema = "AFSC")
vessel <- 148 # Replace with vessel
cruise <- 201001 # Replace with cruise
survey <- "AI" # Region ("BS", "NBS", "GOA", "AI", or "SLOPE")

# Setup directory
source_path <- "G:/RACE_LIGHT/LightData/Data/year_10/ai/v_148/" # Replace with the source directory
mk9process::mk9_setup_dir(source_path = source_path,
                          overwrite = TRUE,
                          survey = survey,
                          vessel = vessel,
                          cruise = cruise)
...
mk9process::mk9_time_offset(channel = channel,
                        survey = survey,
                        cruise = cruise,
                        vessel = vessel)
```
...
mk9process::mk9_cast_times(channel = channel,
                       survey = survey,
                       cruise = cruise,
                       vessel = vessel)
```
...
mk9process::mk9_extinction(channel = channel,
                       survey = survey,
                       cruise = cruise,
                       vessel = vessel)
```
